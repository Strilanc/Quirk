...
function assertThatGateActsLikePermutation(
    ...
    for (let _ of Seq.range(repeats)) {
        ...
        let matrix = Matrix.generateTransition(1 << wireCount, fullPermutation);
        ...
        _assertThatCircuitMutationActsLikePermutation_single(wireCount, updateAction, fullPermutation);
        assertThatCircuitUpdateActsLikeMatrix(updateAction, matrix, 1);
    }
}
 ...
 */
function _assertThatCircuitMutationActsLikePermutation_single(wireCount, updateAction, permutation) {
    let time = Math.random();
    ...
    for (let i = 0; i < ampCount; i++) {
        ...
        if (!outVal.isApproximatelyEqualTo(inVal, 0.001)) {
            ...
            assertThat(outVal).withInfo({i, j, actualIn, actualOut}).isApproximatelyEqualTo(inVal, 0.001);
        }
    }
}
