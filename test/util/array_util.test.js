...
import Au from "src/util/array_util.js"

let ArrayUtilTest = TestCase("ArrayUtilTest");

ArrayUtilTest.prototype.testRepeat = function() {
    assertThat(Au.repeat("a", 0)).isEqualTo([]);
    assertThat(Au.repeat("a", 1)).isEqualTo(["a"]);
    assertThat(Au.repeat("a", 2)).isEqualTo(["a", "a"]);
    assertThat(Au.repeat("b", 5)).isEqualTo(["b", "b", "b", "b", "b"]);
};

ArrayUtilTest.prototype.testRange = function() {
    assertThat(Au.range(0)).iteratesAs();
    assertThat(Au.range(1)).iteratesAs(0);
    assertThat(Au.range(2)).iteratesAs(0, 1);
    assertThat(Au.range(5)).iteratesAs(0, 1, 2, 3, 4);
};

ArrayUtilTest.prototype.testFirstMatchElse = function() {
    assertThat(Au.firstMatchElse([], _ => { throw new Error(); })).isEqualTo(undefined);
    assertThat(Au.firstMatchElse([], _ => { throw new Error(); }, "x")).isEqualTo("x");

    assertThat(Au.firstMatchElse([1, 2, 3], e => e % 2 === 1)).isEqualTo(1);
    assertThat(Au.firstMatchElse([1, 2, 3], e => e % 2 === 0)).isEqualTo(2);
    assertThat(Au.firstMatchElse([1, 2, "a"], e => e === "a")).isEqualTo("a");

    assertThat(Au.firstMatchElse(["a", "b", "c"], _ => true)).isEqualTo("a");
    assertThat(Au.firstMatchElse(["a", "b", "c"], _ => false)).isEqualTo(undefined);

    assertThat(Au.firstMatchElse(["a", "b", "c"], _ => false, "x")).isEqualTo("x");
};
